...
这是卡密、设备绑定、冻结与授权签名的独立服务端。它不和直播录制、AI 处理或用户数据部署在同一台机器上。
...
$body = @{ features = @("basic","export","batch","live_monitor","ai_replay","short_video_ai","lead_radar"); max_devices = 1; note = "客户名称" } | ConvertTo-Json
...
完整服务器、HTTPS 和备份步骤见 [`docs/security/license-server-deployment.md`](../docs/security/license-server-deployment.md)。
