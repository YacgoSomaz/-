...
- AI 设置支持文本/视觉供应商字段，并可扫描 OpenAI 兼容 `/models` 列表。
- 评论线索采集在 Windows 下优先拉起 Edge 持久化 profile，失败后回退 Chromium。
- 授权后台创建卡密时可选择产品与测试有效期，列表会显示完整卡密和到期时间。
- 新安装包不要直接提交普通 Git；发布产物请走 Git LFS 或外部分发渠道。
