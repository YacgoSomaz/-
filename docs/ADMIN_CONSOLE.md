...
- “普通更新”且最低支持版本留空：服务端记录 `0.0.0`，客户端提示新版但允许稍后安装。
- “强制更新”且最低支持版本留空：后台自动使用本次新版本，旧版客户端必须更新后继续使用。
- 显式填写最低支持版本时，客户端会独立比较该字段；因此即使选择普通更新，低于最低支持版本的客户端仍会被强制升级。
- 签名发布后，服务器向该 `product_id` 的 SSE 连接广播事件；客户端随即重新拉取签名清单。SSE 不包含私钥、session 或安装授权。
- 客户端另有 60 秒兜底检查。旧版本没有监听代码，仍只能在启动或旧版已有检查入口发现更新。
...
- 服务端数据库立即生效；复盘虾新版本启动时刷新、运行中默认每 60 秒刷新，明确停用会立即清除本地权益。网络异常仍受签名快照最多 600 秒 TTL 约束，不能把断网当成服务端停用。
...
- 2026-07-16 SSE 更新推送与普通更新语义修复：`/home/ubuntu/recharge-api/backups/20260716094507`
