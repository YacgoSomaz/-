...
def generateSignature(wss, script_file='sign.js'):
    ...
    # 原版用 py_mini_racer(内嵌V8)，但其在 Python 3.14 无预编译轮子；PyExecJS 对 492KB
    # 脚本每次重启 node 又极慢。这里改为调用本地 node 跑 sign_cli.cjs（vm 沙箱复刻纯 V8
    # 环境，避免 sign.js 误走 CommonJS 的 require 分支），一次进程拿到 get_sign 结果。
    import os as _os
    here = _os.path.dirname(_os.path.abspath(__file__))
    cli = _os.path.join(here, "sign_cli.cjs")
    try:
        proc = subprocess.run(
            ["node", cli, md5_param],
        ...
        if not sig:
            print("【X】generateSignature empty; stderr:", (proc.stderr or "")[:200])
            return None
        return sig
    except Exception as e:
        print("【X】generateSignature error:", e)
        return None
