...
def test_commercial_release_scan_blocks_business_python_source(tmp_path):
    ...
    assert any("business Python source" in item for item in findings)
    assert not any("license_runtime.py" in item for item in findings)
...
def test_release_scan_allows_known_bundled_runtime_false_positives(tmp_path):
    ...
    assert scan_release(tmp_path, commercial=True) == []
