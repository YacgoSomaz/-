...
def test_policy_maps_paid_workflows_but_keeps_activation_and_settings_available() -> None:
    ...
    assert feature_for_request("GET", "/api/status") == "live_monitor"
    assert feature_for_request("GET", "/api/diagnostics") == "live_monitor"
    assert feature_for_request("POST", "/api/anchor/resolve") == "live_monitor"
    assert feature_for_request("POST", "/api/rooms/123/start") == "live_monitor"
    assert feature_for_request("POST", "/api/export") == "export"
    assert feature_for_request("GET", "/api/export/summary.xlsx") == "export"
    assert feature_for_request("POST", "/api/ai/report/stream") == "ai_replay"
    assert feature_for_request("GET", "/api/ai/report/view") == "ai_replay"
    ...
    assert feature_for_request("GET", "/api/short-video/cookie/status") == "short_video_ai"
    assert feature_for_request("POST", "/api/short-video/analyze") == "short_video_ai"
    assert feature_for_request("GET", "/api/short-video/parse-cache") == "short_video_ai"
    assert feature_for_request("POST", "/api/comment-leads/run") == "lead_radar"
    assert feature_for_request("POST", "/api/comment-leads/login") == "lead_radar"
    assert feature_for_request("POST", "/api/cookie/remint") == "live_monitor"
    assert feature_for_request("GET", "/api/video-quality") == "live_monitor"
    assert feature_for_request("PUT", "/api/video-quality") == "live_monitor"
    assert feature_for_request("GET", "/api/data/rooms") == "live_monitor"
