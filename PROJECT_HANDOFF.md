...
官网 `https://anyq.site` 已按客户确认的四段顺序上线，不要再把下载卡片、FAQ 或技术 SHA-256 信息直接塞回首页：

1. 顶部账户概览：手机号账户、权益状态、客户端下载入口。
2. 三款产品价格区：复盘虾 2499 元/年、漫剧虾 799 元/年、运营虾 799 元/年（具体金额仍以服务端 `/api/pay/plans` 为准）。
3. 选购结算区：选择产品后展开，提供产品切换、订单摘要和微信支付。
4. 底部信任区与页脚：安心购买、立即开通、专属支持；联系客服辅助安装固定使用微信 `cl17733174657`。

客户端下载、帮助中心仅作为导航后的弹层，不改变现有签名下载与支付接口。当前静态资源版本 `20260716-1300`，生产回滚点为 `/var/www/recharge-site-backups/20260716113826-exact-flow`。本地页面快照在 `.tmp-recharge-site/`（该目录被忽略，不属于复盘虾主线提交）。
